...
(["1"], [], function($__System) {
	...
	$__System.registerDynamic("3", [], true, function($__require, exports, module) {
		...
		module.exports = "<div class=\"easyFormViewer\"><form  novalidate \tng-submit=\"\" id=\"easyFormViewer\" name=\"vm.form\"><formly-form model=\"vm.model\" fields=\"vm.fields\" form=\"vm.form\"> \n\
      <div style=\"text-align: center;margin-top:30px;padding-top:50px;padding-bottom:5px; clear:both;\" ng-hide=\"vm.hideButton()\"> \n\
       <button class=\"btn-black-sm\" ng-click=\"vm.edaSubmitThisDataModel($event,'save')\">Save</button>\n\
       <button class=\"btn-black-sm\" ng-click=\"vm.edaSubmitThisDataModel($event,'exit')\">Save&Exit</button>\n\
       <button class=\"btn-red-sm\" ng-hide=\"vm.hideSubmitButton()\" ng-click=\"vm.edaSubmitThisDataModel($event,'submit',vm.form.$invalid)\">Submit</button>\n\
       </div></formly-form></form></div>";
	...
	$__System.register('5', ['3', '4', '6'], function (_export) {
		...
		function edaFormViewerDirective($modelsTranslator) {
			...
			function linkFct(scope) {
				...
				scope.vm.submitText = scope.edaEasyFormViewerSubmitButtonText || 'Validate';
				scope.vm.cancelText = scope.edaEasyFormViewerCancelButtonText || 'Cancel';
				scope.vm.submitted = false;
				scope.edaEasyFormViewerValidateFlag = false;
				...
				scope.$watch(function () {
					return scope.vm.submitted;
				}, function (newValue, oldValue) {
					if (newValue === true) {
						$("form .ng-valid").removeClass("submitted");
						$("form .ng-invalid").addClass("submitted");
						scope.vm.submitted = false;
					}
				...
				scope.$watch(function () {
					return scope.vm.form.$invalid;
				}, function (newValue, oldValue) {
					scope.edaEasyFormViewerValidateFlag = newValue;
				});
				...
				scope.vm.hideSubmitButton = function(){
					if(scope.hideSubmitButtonFlag === false) { return false; }
					else{ return true; }
				};
				...
				function submitEventWatcher(newSubmitEvent) {
					if (newSubmitEvent === true) {
						if (angular.isFunction(scope.edaEasyFormViewerSubmitFormEvent)) {
							...
							scope.vm.model.action = scope.vm.action;
							scope.vm.model.isInvalid = scope.vm.isInvalid;
			...
			execute: function () {
				...
				edaEasyFormViewerController = (function () {
					...
					_createClass(edaEasyFormViewerController, [{
						...
						value: function edaSubmitThisDataModel(event,action,isInvalid) {
							if(isInvalid){
								this.submitted = true;
							}
	...
	$__System.register('f', ['7', '8', 'e'], function (_export) {
		var _createClass, _classCallCheck, resetNyaSelect, getEmptyConfigModelResult, resetDataModel, getErrorObject, getMessageObject, resetFormlyModel, addOneColumnHeader, addOneColumnControl, addTwoColumnControl, addThreeColumnControl, addFourColumnControl, addFiveColumnControl, addSixColumnControl, MODEL_TRANSLATOR_SERVICE, $modelsTranslator;
...
//			_export("validationTemplate", validationTemplate);
	...
	$__System.register('12', ['11'], function (_export) {
		...
		function edaEasyFormViewerConfig(formlyConfigProvider, formlyApiCheck, onProd) {
